...
class Controller {
    ...
    initPlayButton() {
        ...
        // REW 10s
        this.player.template.mobileBackwardButton.addEventListener(
            "click",
            () => {
                ...
                this.player.notice(
                    `${this.player.tran(
                        "REW"
                    )} ${this.mobileBackwardTime.toFixed(0)} ${this.player.tran(
                        "s"
                    )}`
                );
                // extend count reset
        ...
        // FF 10s
        this.player.template.mobileForwardButton.addEventListener(
            "click",
            () => {
                this.mobileForwardTime += 10;
                this.player.seek(this.player.video.currentTime + 10);
                this.player.notice(
                    `${this.player.tran("FF")} ${this.mobileForwardTime.toFixed(
                        0
                    )} ${this.player.tran("s")}`
                );
                // extend count reset
    ...
    initHighlights() {
        this.player.on("durationchange", () => {
            if (
                this.player.video.duration !== 1 &&
                this.player.video.duration !== Infinity
            ) {
                if (this.player.options.highlight) {
                    const highlights =
                        this.player.template.playedBarWrap.querySelectorAll(
                            ".dplayer-highlight"
                        );
                    ...
                    for (
                        let i = 0;
                        i < this.player.options.highlight.length;
                        i++
                    ) {
                        if (
    ...
    initPlayedBar() {
        ...
        const thumbMove = (e) => {
            ...
            const duration = utils.getVideoDuration(
                this.player.video,
        ...
        this.player.template.playedBarWrap.addEventListener(
            ...
            (e) => {
                ...
                if (duration) {
                    const px =
                        this.player.template.playedBarWrap.getBoundingClientRect()
                            .left;
                    ...
                    this.player.template.playedBarTime.innerText =
                        utils.secondToTime(time);
                    this.player.template.playedBarTime.classList.remove(
                        "hidden"
                    );
                }
            }
        ...
        if (!utils.isMobile) {
            ...
            this.player.template.playedBarWrap.addEventListener(
                ...
                () => {
                    if (this.player.video.duration) {
                        this.thumbnails && this.thumbnails.hide();
                        this.player.template.playedBarTime.classList.add(
                            "hidden"
                        );
                    }
                }
            ...
            this.player.template.pipButton.addEventListener("click", () => {
                if (!document.pictureInPictureElement) {
                    this.player.video.requestPictureInPicture().catch(() => {
                        this.player.notice(
                            "Error: Picture-in-Picture is not supported."
                        );
                    });
                } else {
    ...
    initVolumeButton() {
        ...
        const volumeUp = () => {
            ...
            this.player.template.volumeButton.classList.remove(
        ...
        this.player.template.volumeBarWrapWrap.addEventListener(
            ...
            () => {
                ...
                this.player.template.volumeButton.classList.add(
                    "dplayer-volume-active"
                );
            }
        );
                    ...
                    (blob) => {
                        ...
                        if (!this.player.options.screenshotOfEventTriggerOnly) {
                            ...
                            if (typeof link.download === "undefined") {
                                this.player.notice(
                                    "Error: Screenshot download is not supported."
                                );
                                return;
                            }
                            ...
                            const month = ("0" + (today.getMonth() + 1)).slice(
                                -2
                            );
    ...
    initSubtitleButton() {
        if (this.player.options.subtitle) {
            this.player.events.on("subtitle_show", () => {
                this.player.template.subtitleButton.ariaLabel =
